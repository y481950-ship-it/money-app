...
<body class="bg-gray-100 text-gray-800 pb-20">
    ...
    <div id="main-app" class="hidden max-w-md mx-auto min-h-screen bg-white shadow-md flex flex-col">
        <header class="bg-blue-600 text-white p-4 sticky top-0 z-10 flex justify-between items-center shadow">
            <div>
                ...
                <div class="flex items-center gap-2 mt-1">
                    <button type="button" id="btn-prev-month" class="text-xs bg-blue-700 hover:bg-blue-800 px-2 py-0.5 rounded font-bold">◀</button>
                    <span id="current-month" class="text-xs text-white font-bold tracking-wide"></span>
                    <button type="button" id="btn-next-month" class="text-xs bg-blue-700 hover:bg-blue-800 px-2 py-0.5 rounded font-bold">▶</button>
                </div>
            </div>
        ...
        <div class="p-3 bg-white border-b">
            <div class="bg-gray-50 border border-gray-200 p-2.5 rounded-lg shadow-sm">
                <h3 class="text-xs font-bold text-gray-700 mb-1">📊 지난달 대비 소비 추이</h3>
                <div id="comparison-content" class="text-xs text-gray-600 space-y-0.5">
                    데이터를 계산 중입니다...
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function renderSummary() {
            ...
            let prevFood = 0;
            prevTxs.forEach(t => {
                if (t.type === 'expense' && t.is_fixed === 0) {
                    prevVariable += Number(t.amount);
                    if (t.category === '식비' || t.category === '외식') {
                        prevFood += Number(t.amount);
                    }
                }
            });
            ...
            let varBadge = diffVar <= 0
                ? `<span class="text-emerald-600 font-bold">${Math.abs(diffVar).toLocaleString()}원 절약 중 ▼</span>`
                : `<span class="text-red-500 font-bold">${diffVar.toLocaleString()}원 초과 중 ▲</span>`;

            let foodBadge = diffFood <= 0
                ? `<span class="text-emerald-600 font-bold">${Math.abs(diffFood).toLocaleString()}원 절약 중 ▼</span>`
                : `<span class="text-red-500 font-bold">${diffFood.toLocaleString()}원 초과 중 ▲</span>`;
